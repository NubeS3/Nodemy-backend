...
categoryRoute.post("/categories", adminAuthentication, rolesValidation(['Admin']), requestValidation(createCategoryRequest), async (req, res) => {
  try {
...
categoryRoute.delete("/categories/:id", adminAuthentication, rolesValidation(['Admin']), requestValidation(deleteCategoryRequest), async (req, res) => {
  try {
    const category = await Category.findById(req.params.id);
    if (!category) {
      return res.status(404).send({error: "Found no category"});
...
categoryRoute.patch("/categories/:id", adminAuthentication, rolesValidation(['Admin']), requestValidation(updateCategoryRequest), async (req, res) => {
  try {
    const category = await Category.findById(req.params.id);
    if (!category) {
      return res.status(404).send({error: "Found no category"});
